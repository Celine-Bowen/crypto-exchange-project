<script setup>
  ...
  async function handleAuth({ mode, payload }) {
    ...
    try {
      ...
      const profileData =
        data.profile?.data ?? data.profile ?? data.data ?? null;
      ...
      setFlash(
        mode === "register"
          ? "Account created with demo USD funds."
          : "Welcome back!"
      );
  ...
  <div class="min-h-screen bg-slate-950 text-slate-100">
    ...
    <main class="mx-auto max-w-6xl space-y-6 px-4 pb-12 pt-6">
      ...
      <div v-else class="space-y-6">
        <p v-if="bootstrapLoading" class="text-sm text-slate-400">
          Syncing your wallet...
        </p>
